...
class FLOFA_Trainer(ABC):
    def __init__(
        ...
        # The legacy trainer calls its second stream ``test``. In clean mode
        # that stream must actually be the validation split; fail closed if a
        # caller accidentally wires the official test loader into selection.
        if getattr(args, "use_train_pkl", False):
            invalid_loaders = [
                loader
                for loader in test_data_local_dict.values()
                if loader is not None and getattr(loader, "split_name", None) != "validation"
            ]
            if invalid_loaders:
                raise RuntimeError(
                    "Clean training requires validation loaders for checkpoint selection; "
                    "the official test loader was supplied instead."
                )
        ...
        ### NEW: Add the DeepMAD parameters to the sampler_args dictionary ###
        ...
        self.prev_best = float("inf") if self.args.dataset == "ptb" else 0.0
        if resume_state is not None:
            self.prev_best = resume_state["best_metric"]
        ...
        if resume_state is not None and self.feddyn:
            saved_server_state = resume_state.get("server_state")
            if saved_server_state is None:
                raise ValueError("FedDyn resume checkpoint is missing server_state")
            saved_server_state = np.asarray(saved_server_state, dtype="float32")
            if saved_server_state.shape != self.server_state.shape:
                raise ValueError(
                    "FedDyn server_state shape mismatch: "
                    f"checkpoint={saved_server_state.shape}, current={self.server_state.shape}"
                )
            self.server_state = saved_server_state.copy()
    ...
    def _checkpoint_training_state(self, completed_round):
        state = {
            "completed_round": int(completed_round),
            "next_round": int(completed_round) + 1,
            "best_metric": float(self.prev_best),
        }
        if self.feddyn:
            state["server_state"] = self.server_state
        return state

    def _checkpoint_mean_metric(self, subnet_test_metric_map):
        """Return the mean metric used to select the best checkpoint.

        Periodic evaluation covers ``diverse_subnets`` while checkpoint
        selection may use an independent ``ckpt_subnets`` list.  Preserve the
        independent evaluation when those lists differ, but avoid evaluating
        the same deterministic validation architectures twice when they are
        an exact ordered match.
        """
        evaluated_subnet_ids = list(self.args.diverse_subnets)
        evaluated_subnets = [
            self.args.diverse_subnets[subnet_id]
            for subnet_id in evaluated_subnet_ids
        ]
        checkpoint_subnets = self.args.ckpt_subnets
        if checkpoint_subnets is None:
            checkpoint_subnets = evaluated_subnets

        can_reuse = (
            checkpoint_subnets == evaluated_subnets
            and all(
                subnet_id in subnet_test_metric_map
                for subnet_id in evaluated_subnet_ids
            )
        )
        if can_reuse:
            logging.info(
                "Reusing %d periodic validation metrics for checkpoint selection.",
                len(evaluated_subnet_ids),
            )
            return sum(
                subnet_test_metric_map[subnet_id]
                for subnet_id in evaluated_subnet_ids
            ) / len(evaluated_subnet_ids)

        logging.info(
            "Periodic validation metrics cannot be reused for checkpoint "
            "selection; running the independent checkpoint evaluation."
        )
        mean_metric = 0.0
        for subnet_arch in checkpoint_subnets:
            ...
            if self.args.dataset == 'ptb':
                mean_metric += test_metrics["test_ppl"]
            else:
                mean_metric += (
                    test_metrics["test_correct"] / test_metrics["test_total"]
                )
        return mean_metric / len(checkpoint_subnets)
    ...
    def train(self):
        ...
        self.server_model.upload_checkpoint("best_checkpoint_supernet.pt")
        self.server_model.upload_checkpoint("latest_round_model.pt")
    ...
    def train_one_round(self, round_num, local_ep=None, **kwargs):
        ...
        # Test at the requested interval and always on the final round. The
        # resulting mean is also used to update the single best checkpoint.
        if (
            round_num == self.args.comm_round - 1
            or round_num % self.args.frequency_of_the_test == 0
        ):
            ...
            mean_acc = self._checkpoint_mean_metric(subnet_test_acc_map)
            if self.args.dataset == 'ptb':
                wandb.log(
                ...
                if mean_acc < self.prev_best:
                    ...
                    self.server_model.save(
                        ...
                        self._checkpoint_training_state(round_num),
                    )
            else:
                wandb.log(
        ...
        # Overwrite the same latest file every round; no round-specific files
        # are retained on local storage.
        latest_model_name = "latest_round_model.pt"
        self.server_model.save(
            latest_model_name,
            self._checkpoint_training_state(round_num),
        )
