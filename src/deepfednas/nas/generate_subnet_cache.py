...
try:
    from deepfednas.nas.deepfednas_fitness_maximizer import (
        ...
        calculate_paper_deepmad_entropy_objective
    )
...
def run_single_ga_job(job_args):
    """A worker function that runs one GA search for a specific MACs target."""
    ...
    if best_arch:
        ...
        # --- NEW: Calculate effectiveness and entropy ---
        ...
        entropy, _ = calculate_paper_deepmad_entropy_objective(
            best_arch['d'], best_arch['e'], best_arch['w_indices'],
            width_mult_options=width_choices,
            arch_config_params=arch_config_params
        )
...
def main(args):
    print("--- Flexible Fitness Dataset Generation Script ---")
    ...
    job_args = [(mac, np.random.randint(0, 1e7), arch_config_params, width_multiplier_choices, expansion_ratio_choices, depth_choices, args.rho0_constraint, args.ga_pop_size, args.ga_generations, args.ga_mutate_p) for mac in mac_targets]

    # --- Run parallel generation ---
    print(f"Starting parallel generation with {args.num_processes} processes...")
    with multiprocessing.Pool(processes=args.num_processes) as pool:
        with tqdm(total=num_samples_needed, desc="Generating Samples") as pbar:
            for i, result in enumerate(pool.imap_unordered(run_single_ga_job, job_args)):
                if result:
                ...
                if (i + 1) % args.save_interval == 0 and collected_data:
                    ...
                    tqdm.write(f"  -> Saved {len(collected_data)} samples to {args.output_csv}")
