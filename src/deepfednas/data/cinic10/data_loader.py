...
def _data_transforms_cinic10(
    ...
    if augmentation not in ("basic", "mixaug"):
        raise ValueError(
            f"Unsupported CINIC-10 augmentation {augmentation!r}; "
    ...
    # Validation/test transform: deterministic and free of augmentation.
...
def load_cinic10_data(datadir):
    ...
    _val_dir = datadir + str('/val')
    train_transform, val_transform = _data_transforms_cinic10()
    trainset = ImageFolderTruncated(_train_dir, transform=train_transform)
    valset = ImageFolderTruncated(_val_dir, transform=val_transform)
    X_train, y_train = trainset.imgs, trainset.targets
    X_val, y_val = valset.imgs, valset.targets
...
def partition_data(dataset, datadir, partition, n_nets, alpha):
    ...
    X_train, y_train, X_val, y_val = load_cinic10_data(datadir)
    ...
    return X_train, y_train, X_val, y_val, net_dataidx_map, traindata_cls_counts


# for centralized training
def get_dataloader(
    dataset, datadir, train_bs, test_bs=256, dataidxs=None,
    augmentation="basic",
    randaugment_num_ops=2, randaugment_magnitude=6,
):
    return get_dataloader_cinic10(
        datadir, train_bs, test_bs, dataidxs, augmentation,
...
def get_dataloader_cinic10(
    datadir, train_bs, test_bs, dataidxs=None, augmentation="basic",
    randaugment_num_ops=2, randaugment_magnitude=6,
):
    ...
    val_ds = dl_obj(valdir, transform=transform_test)

    train_dl = data.DataLoader(dataset=train_ds, batch_size=train_bs, shuffle=True, drop_last=False)
    val_dl = data.DataLoader(dataset=val_ds, batch_size=test_bs, shuffle=False, drop_last=False)
...
def get_dataloader_test_cinic10(
    ...
    test_ds = dl_obj(testdir, dataidxs=dataidxs_test, transform=transform_test)
...
def load_partition_data_distributed_cinic10(process_id, dataset, data_dir, partition_method, partition_alpha,
    ...
    X_train, y_train, X_val, y_val, net_dataidx_map, traindata_cls_counts = partition_data(dataset,
                                                                                           data_dir,
                                                                                           partition_method,
                                                                                           client_number,
                                                                                           partition_alpha)
...
def load_partition_data_cinic10(
    ...
    batch_size, val_batch_size=256, augmentation="basic",
    randaugment_num_ops=2, randaugment_magnitude=6,
):
    X_train, y_train, X_val, y_val, net_dataidx_map, traindata_cls_counts = partition_data(dataset,
                                                                                           data_dir,
