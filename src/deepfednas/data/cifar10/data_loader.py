...
def partition_data(dataset, datadir, partition, n_nets, alpha, load_from_pkl):
...
def get_dataloader(dataset, datadir, train_bs, test_bs, dataidxs=None, load_from_pkl=False):
    return get_dataloader_CIFAR10(datadir, train_bs, test_bs, dataidxs, load_from_pkl)
...
def get_dataloader_CIFAR10(datadir, train_bs, test_bs, dataidxs=None, load_from_pkl=False):
    ...
    if load_from_pkl:
        with open(datadir+'/train.pkl', 'rb') as file:
            train_tuple = pickle.load(file)
            train_ds.data, train_ds.target = train_tuple[0][dataidxs], train_tuple[1][dataidxs]
...
def load_partition_data_cifar10(dataset, data_dir, partition_method, partition_alpha, client_number, batch_size, val_batch_size=256, load_from_pkl=False):
    X_train, y_train, X_test, y_test, net_dataidx_map, traindata_cls_counts = partition_data(dataset,
                                                                                             data_dir,
                                                                                             partition_method,
                                                                                             client_number,
    ...
    train_data_global, test_data_global = get_dataloader(dataset, data_dir, batch_size, val_batch_size, load_from_pkl=load_from_pkl)
    logging.info("train_dl_global number = " + str(len(train_data_global)))
    logging.info("test_dl_global number = " + str(len(test_data_global)))
