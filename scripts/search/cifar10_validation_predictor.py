#!/usr/bin/env python3
"""Build the CIFAR-10 baseline accuracy predictor from the fixed val.pkl split.

This script deliberately does not load the official CIFAR-10 test set.  It
samples baseline-supernet architectures, evaluates them on the fixed
``data/cifar10/val.pkl`` split, and trains the predictor used by the locked
architecture-search driver.
"""
...
CHECKPOINT = REPO_ROOT / "checkpoints/cifar10_alpha100_c0.4/range_matched_random/seed0/best_checkpoint_supernet.pt"
DATA_DIR = REPO_ROOT / "data/cifar10"
OUTPUT_DIR = REPO_ROOT / "outputs/cifar10/predictor"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--architecture-seed", type=int, default=20260812)
    parser.add_argument("--predictor-split-seed", type=int, default=20260813)
    parser.add_argument("--epochs", type=int, default=400)
    parser.add_argument("--learning-rate", type=float, default=1e-4)
    parser.add_argument("--architecture-batch-size", type=int, default=1024)
...
def load_validation_dataset(data_dir: Path) -> ArrayDataset:
    with (data_dir / "val.pkl").open("rb") as handle:
        images, labels = pickle.load(handle)
    dataset = ArrayDataset(np.asarray(images), np.asarray(labels))
    if len(dataset) != 5000:
        raise RuntimeError(f"Expected 5,000 CIFAR-10 validation images, found {len(dataset)}")
    return dataset
...
def train_predictor(features, targets, args, device, sampling_bins=None):
    tensors = TensorDataset(
        torch.tensor(features, dtype=torch.float32),
        torch.tensor(targets, dtype=torch.float32).unsqueeze(1),
    )
    train_size = int(0.9 * len(tensors))
    heldout_size = len(tensors) - train_size
    generator = torch.Generator().manual_seed(args.predictor_split_seed)
    train_set, heldout_set = random_split(tensors, [train_size, heldout_size], generator=generator)
    ...
    for _ in tqdm(range(args.epochs), desc="Training CIFAR-10 validation predictor"):
        model.train()
    ...
    metrics = {
        ...
        "heldout_rank_correlation": float(np.corrcoef(ranks_pred, ranks_actual)[0, 1]),
...
def load_initial_rows(path: Path, args: argparse.Namespace, sampling_plan: list[tuple]) -> tuple[list[dict], set[str]]:
    if not path.is_file():
...
def main() -> None:
    ...
    validation_dataset = load_validation_dataset(args.data_dir)
    loader = DataLoader(
        validation_dataset, batch_size=args.architecture_batch_size,
    ...
    rows, seen = (load_initial_rows(args.initial_dataset, args, sampling_plan)
    ...
    with tqdm(
        ...
        desc="Evaluating CIFAR-10 validation architectures",
    ) as progress:
        for bin_name, lower, upper, target_count, width_floor in sampling_plan:
            ...
            while accepted < target_count:
                ...
                row = {f"feature_{i}": float(value) for i, value in enumerate(features)}
                row.update({
                ...
                del client
                if len(rows) % 100 == 0:
                    write_csv_atomic(pd.DataFrame(rows), args.output_dir / "predictor_dataset.partial.csv")
    ...
    predictor, metrics = train_predictor(
        ...
        dataframe["accuracy"].to_numpy(dtype=np.float32), args, device,
        dataframe["sampling_bin"].to_numpy(),
    )
    ...
    manifest_path = args.output_dir / "validation_split_provenance.json"
    ...
    manifest_path.write_text(json.dumps({
        "dataset": "CIFAR-10",
        "split": "val",
        "path": str((args.data_dir / "val.pkl").resolve()),
        "sample_count": len(validation_dataset),
        "selection": "All 5,000 fixed validation samples; no subset sampling",
        "sha256": sha256_file(args.data_dir / "val.pkl"),
    }, indent=2) + "\n")
    metrics_path.write_text(json.dumps(metrics, indent=2) + "\n")
    provenance = {
        "completed": True,
        "dataset": "CIFAR-10",
        "split": "fixed data/cifar10/val.pkl",
        "test_data_loaded": False,
        "validation_sample_count": len(validation_dataset),
        "validation_transform": "ToTensor(); Normalize(CIFAR10_MEAN, CIFAR10_STD); no augmentation",
        ...
        "architecture_batch_size": args.architecture_batch_size,
        ...
        "validation_split_provenance_sha256": sha256_file(manifest_path),
        "metrics": metrics,
