#!/usr/bin/env python3
"""Build the CIFAR-100 baseline accuracy predictor from fixed val.pkl.

This phase loads only the fixed CIFAR-100 validation pickle. It evaluates
baseline-supernet architectures on those 5,000 validation images and trains
the predictor used by the locked search. The official CIFAR-100 test split is
never loaded here.
"""
...
CHECKPOINT = REPO_ROOT / "checkpoints/cifar100_alpha100_c0.4/range_matched_random/seed0/best_checkpoint_supernet.pt"
DATA_DIR = REPO_ROOT / "data/cifar100"
OUTPUT_DIR = REPO_ROOT / "outputs/cifar100/predictor"

CIFAR100_MEAN = [0.5071, 0.4865, 0.4409]
CIFAR100_STD = [0.2673, 0.2564, 0.2762]

MAC_BINS = (
    ("0.458-0.95B", 458_330_112, 950_000_000),
    ("0.95-1.45B", 950_000_000, 1_450_000_000),
    ("1.45-2.45B", 1_450_000_000, 2_450_000_000),
    ("2.45-3.403B", 2_450_000_000, 3_403_554_816),
)
...
def load_validation_dataset(data_dir: Path) -> ArrayDataset:
    ...
    if len(dataset) != 5000:
        raise RuntimeError(f"Expected 5,000 CIFAR-100 validation images, found {len(dataset)}")
    return dataset
...
def train_predictor(features, targets, args, device, sampling_bins=None):
    ...
    for _ in tqdm(range(args.epochs), desc="Training CIFAR-100 validation predictor"):
        model.train()
...
def main() -> None:
    ...
    with tqdm(
        ...
        desc="Evaluating CIFAR-100 validation architectures",
    ) as progress:
    ...
    provenance = {
        "completed": True,
        "dataset": "CIFAR-100",
        "split": "fixed data/cifar100/val.pkl",
        "test_data_loaded": False,
        "validation_sample_count": len(validation_dataset),
        "validation_transform": "ToTensor(); Normalize(CIFAR100_MEAN, CIFAR100_STD); no augmentation",
