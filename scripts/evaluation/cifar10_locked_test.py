#!/usr/bin/env python3
"""Evaluate a completed CIFAR-10 architecture-search manifest on test data.

This is intentionally a separate phase from architecture selection.  It
refuses incomplete or hash-mismatched manifests and performs no ranking,
tuning, or architecture replacement.
"""
...
from deepfednas.data.cifar10.datasets import CIFAR10_truncated
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--search-dir", type=Path, default=REPO_ROOT / "outputs/cifar10/search")
    parser.add_argument("--data-dir", type=Path, default=REPO_ROOT / "data/cifar10")
    ...
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help="Explicitly replace an existing completed test evaluation.",
    )
...
def main() -> None:
    ...
    test_dataset = CIFAR10_truncated(str(args.data_dir), train=False, transform=test_transform, download=False)
    if len(test_dataset) != 10000:
        raise RuntimeError(f"Expected 10,000 CIFAR-10 test images, found {len(test_dataset)}")
    ...
    provenance = {
        ...
        "transform": "ToTensor(); Normalize(CIFAR10_MEAN, CIFAR10_STD); no augmentation",
