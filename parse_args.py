...
def add_args(parser):
    ...
    # ------------------ DeepMAD-related NAS Sampler Settings ------------------
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional JSON manifest containing the exact client partition "
            "to load and validate before training."
        ),
    )
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Expected partition seed recorded in the client manifest.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--augmentation",
        choices=("basic", "mixaug"),
        default="basic",
        help=(
            "Training image augmentation. 'basic' preserves the existing "
            "dataset pipeline; 'mixaug' adds configurable RandAugment and "
            "requires a non-'none' --mix_aug_mode."
        ),
    )
    parser.add_argument(
        "--randaugment_num_ops",
        type=int,
        default=2,
        help="Number of sequential RandAugment operations used by MixAug.",
    )
    parser.add_argument(
        "--randaugment_magnitude",
        type=int,
        default=6,
        help="RandAugment magnitude in the torchvision range [0, 30].",
    )
    parser.add_argument(
        "--mix_aug_mode",
        choices=("none", "alternating", "mixup", "cutmix"),
        default="none",
        help=(
            "Optional batch-level label mixing used with --augmentation "
            "mixaug; 'alternating' chooses Mixup or CutMix equiprobably per "
            "batch."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Beta-distribution alpha for CutMix.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        choices=['static', 'TS_optimal_path', 'TS_range_matched_random', 'TS_cached_entropy_maximizer', 'TS_entropy_maximizer', 'dynamic', 'all_random', 'TS_all_random', 'sandwich_all_random', 'TS_compound', 'max_sample_count', 'multi_sandwich', 'TS_KD', 'PS'],
    ...
    parser.add_argument(
        "--wandb_fresh_run_on_resume",
        action="store_true",
        help=(
            "Resume training state from the local checkpoint but start a fresh "
            "W&B run segment. This avoids resumed-run heartbeat problems without "
            "changing training, RNG, or checkpoint state."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Faithfully continue training from --local_model_ckpt_path. The next "
            "round and best validation score are read from the checkpoint, all "
            "RNG states are restored, and training-critical arguments are checked."
        ),
    )
    parser.add_argument(
        "--cli_subnet_track",
    ...
    parser.add_argument(
        ...
        type=int,
        default=500,
        help="Legacy checkpoint frequency; retained for command-line compatibility.",
    )
    parser.add_argument(
        "--best_model_freq",
        type=int,
        default=1000,
        help="Legacy best-checkpoint interval; retained for command-line compatibility.",
    )
    parser.add_argument(
        "--checkpoint_dir",
        type=str,
        default=None,
        help=(
            "Local directory for best_checkpoint_supernet.pt and "
            "latest_round_model.pt. If unset, use the W&B run directory "
            "when uploads are enabled, or a local checkpoints fallback."
        ),
    )
    parser.add_argument(
        "--wandb_upload_checkpoints",
        dest="wandb_upload_checkpoints",
        action="store_true",
        help="Upload the final best and latest checkpoint files to W&B.",
    )
    parser.add_argument(
        "--no_wandb_upload_checkpoints",
        dest="wandb_upload_checkpoints",
        action="store_false",
        help="Keep checkpoint files local and do not upload weights to W&B.",
    )
    parser.set_defaults(wandb_upload_checkpoints=True)
    parser.add_argument(
        "--custom_config",
        type=json.loads,
        default=None,
